...
export function EgyptToursTool() {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(egyptTours[0]?.id ?? null)
  ...
  return (
    <div className="tool-page egypt-tool">
      <header className="page__header egypt-tool__intro">
        ...
        <h1>埃及團比較</h1>
        <p className="muted">
          整理 9 條 gouni.tours 短網址行程：價格、出發、航空、轉機與重點差異。價格與機位以查詢當下出發團為準。
        </p>
      </header>
      ...
      <WindowFrame
        ...
        toolbar={
          <div className="egypt-tool__toolbar">
            <div className="egypt-tool__seg" role="group" aria-label="檢視">
              <button
                type="button"
                className={view === 'compare' ? 'is-on' : undefined}
              ...
              <button
                type="button"
                className={view === 'cards' ? 'is-on' : undefined}
        ...
        footer={`${filtered.length} 團顯示 · 來源短網址 gouni.tours`}
      >
      ...
      {selected ? (
        ...
        >
          <article className="egypt-detail">
            ...
            <div className="egypt-detail__grid">
              <section>
                <h3>出發與航班</h3>
